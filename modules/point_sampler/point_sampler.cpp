...
void PointSampler::random(int amount, uint64_t seed) {
	// uint64_t start = Time::get_singleton()->get_ticks_usec();
	point_samples = Vector<Vector3>();
	normal_samples = Vector<Vector3>();
	point_samples.resize(amount);
	normal_samples.resize(amount);
	...
	auto task_id = WorkerThreadPool::get_singleton()->add_native_group_task([](void *userdata, uint32_t idx) {
		...
		Vector3 ba = b - a;
		Vector3 ca = c - a;
		t.point_samples->set(idx, point);
		t.normal_samples->set(idx, ba.cross(ca));
	},
			&task_data, amount);
...
Vector<Vector3> PointSampler::get_point_samples() const {
	return point_samples;
}

Vector<Vector3> PointSampler::get_normal_samples() const {
...
void PointSampler::poisson(float radius, float density, uint64_t seed) {
	float total_area = this->cumulative_areas[this->cumulative_areas.size() - 1];
	...
	auto task_id = WorkerThreadPool::get_singleton()->add_native_group_task([](void *userdata, uint32_t idx) {
		...
		(*t.samples)[idx] = Sample{ pos, normal, cell_id };
	},
	...
	for (int i = 0; i < 3 * 3 * 3; i++) {
		...
		auto task_id = WorkerThreadPool::get_singleton()->add_native_group_task([](void *userdata, uint32_t idx) {
			...
			for (uint32_t i = 0; i < t.max_density; i++) {
				...
				auto normal = (*t.oversamples)[si].normal;
	...
	point_samples = Vector<Vector3>();
	normal_samples = Vector<Vector3>();
	point_samples.resize(expected_samples * 1.1f);
	normal_samples.resize(expected_samples * 1.1f);
	{
		int i = 0;
		for (const auto &kv : ggrid) {
			if (kv.value.sample != Vector3(FLT_MAX, FLT_MAX, FLT_MAX)) {
				point_samples.set(i, kv.value.sample);
				normal_samples.set(i, kv.value.normal);
				i += 1;
			}
		}
		point_samples.resize(i);
		normal_samples.resize(i);
	}
...
void PointSampler::_bind_methods() {
	...
	ClassDB::bind_method(D_METHOD("get_point_samples"), &PointSampler::get_point_samples);
	ClassDB::bind_method(D_METHOD("get_normal_samples"), &PointSampler::get_normal_samples);
}

PointSampler::PointSampler() {
	this->faces = Vector<Face3>();
	this->cumulative_areas = Vector<float>();
	this->point_samples = Vector<Vector3>();
	this->normal_samples = Vector<Vector3>();
}
