...
void light_process_directional_shadow(uint i, vec3 vertex, vec3 geo_normal, highp vec2 directional_shadow_pixel_size, inout uint shadow0, inout uint shadow1) {
#ifndef SHADOWS_DISABLED
...
float sample_directional_shadow(uint light_idx, vec3 vertex) {
    ...
    light_process_directional_shadow(light_idx, vertex, normalize(normal_interp), scene_data_block.data.directional_shadow_pixel_size, shadow0, shadow1);
...
void light_process_omni(uint idx, vec3 vertex, vec3 eye_vec, vec3 normal, vec3 vertex_ddx, vec3 vertex_ddy, vec3 f0, uint orms, float taa_frame_count, vec3 albedo, inout float alpha, vec2 screen_uv,
	...
	#else
		float light_attenuation = omni_attenuation * shadow;
	#endif
	light_compute(normal, light_rel_vec_norm, eye_vec, size, color, false, light_attenuation, f0, orms, omni_lights.data[idx].specular_amount, albedo, alpha, screen_uv,
#ifdef LIGHT_BACKLIGHT_USED
			backlight,
#endif
#ifdef LIGHT_TRANSMITTANCE_USED
...
void light_process_spot(uint idx, vec3 vertex, vec3 eye_vec, vec3 normal, vec3 vertex_ddx, vec3 vertex_ddy, vec3 f0, uint orms, float taa_frame_count, vec3 albedo, inout float alpha, vec2 screen_uv,
	...
	#else
		float light_attenuation = spot_attenuation * shadow;
	#endif

	light_compute(normal, light_rel_vec_norm, eye_vec, size, color, false, light_attenuation, f0, orms, spot_lights.data[idx].specular_amount, albedo, alpha, screen_uv,
#ifdef LIGHT_BACKLIGHT_USED
			backlight,
#endif
#ifdef LIGHT_TRANSMITTANCE_USED
